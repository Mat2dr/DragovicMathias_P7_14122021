    ...
    for (let i = 0; i < recettes.length; i++) {
        if (!!value) {
            //Si il y a une valeur dans la recherche principal
             if (recettes[i].name.toLowerCase().includes(value) || recettes[i].description.toLowerCase().includes(value)) {
                //Si il y a des tags active
                if (this.ingredientsActiveTags || this.ustensilesActiveTags || this.appareilActiveTags) {
                    if (ingredientsFilter(recettes[i]) && ustensilesFilter(recettes[i]) && appareilFilter(recettes[i])) {
                        recetteFiltered.push(recettes[i]);
                    }
                } else {
                    recetteFiltered.push(recettes[i]);
                }
...
function ingredientsFilter(recette) {
    let ingredientArray = [];
...
function ustensilesFilter(recette) {
    let ustensilesArray = [];
...
function appareilFilter(recette) {
        let appareilArray = [];
        //Loop dans les ingredients de toutes le recettes 
